...
let jsonFilePath = './secrets/default.copie.json';
...
const body = `$body = @{cid = "QmRMMouewgE4SpoUkGcv6rTdSZ59zcSV24PwhCpY1NEPvL", data = @{secretSalt = "SALTY_BAE"} } | ConvertTo-Json`
try {
  ...
  // const command = `Invoke-WebRequest -Uri "https://wapo-testnet.phala.network/vaults" -Headers $headers -Method Post -Body $body`;
  // const command = `curl -X POST ${gatewayUrl}/vaults -H "Content-Type: application/json" -d '{"cid": "${cid}", "data": ${JSON.stringify(secrets)}}'`;
  const command = `curl -X POST ${gatewayUrl}/vaults ${headers} -d '${body}'`;
